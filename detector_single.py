...
def merge_detections_by_heatmap(dets, image_shape, thresh=0.5):
# Merge overlapping detections into one box via heatmap thresholding.
    if not dets:
        return None
...
def main():
    ...
    while not shut_it_down:
    # user interface loop. no actual work here, just to grab an image
        ...
        if choice == '1':
            ...
            while True:
                ...
                cv2.imshow(
                    ...
                    frame
                    )   # this is very ugly but linter demands it
                key = cv2.waitKey(1) & 0xFF
    ...
    # after capturing or loading an image into captured_image, run detection
        ...
        dets = [tuple(int(v) if i < 4 else float(v) for i, v in enumerate(item))
            for item in dets
        ]
