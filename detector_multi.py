...
CONF_THRESHOLD = 0.92           # only keep windows >= this apple-probability (using apple and windows like that feels weird ngl)
...
def split_and_filter_apples(dets, image, heat_thresh=0.1, min_area_frac=0.001, sky_v_thresh=0.9, apple_hue_ranges=[(0,10),(160,180)], debug=False):
	h, w = image.shape[:2]
	total_area = h*w
	...
	if debug:
		...
		cv2.imshow("Markers", (markers.astype(np.float32)/markers.max()*255).astype(np.uint8))
		cv2.waitKey(0)
	...
	if debug:
		...
		cv2.imshow("Watershed markers", (markers.astype(np.float32)/markers.max()*255).astype(np.uint8))
		cv2.waitKey(0)
		cv2.destroyAllWindows()

	return boxes

def resizinator(image, target_size=(640, 480)):	# otherwise the pyramid detection will take ages
	# Resize `image` so that it best fits within `target_size`, preserving aspect ratio.
	# If the image is larger than the target in at least one dimension, it will be shrunk.
	...
	# Otherwise both dims are smaller than target (ratio ≥ 1), so we enlarge by the larger ratio.
...
def main():
	...
	while not shut_it_down:		# user interface loop. no actual work here, just to grab an image ffs
	...
	if not shut_it_down:		# after capturing or loading an image into captured_image, run detection
		...
		boxes = split_and_filter_apples(dets, captured_image, heat_thresh=0.05, min_area_frac=0.002, sky_v_thresh=0.9, apple_hue_ranges=[(0,10),(160,180)],
			debug=True)
